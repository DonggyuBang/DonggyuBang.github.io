document.addEventListener('DOMContentLoaded',async()=>{
  ...
  const order=['Advisor','Research Professors','Postdoctoral Researchers','Ph.D. Students','Integrated M.S./Ph.D. Students','M.S. Students','Administrative Staff'];
  ...
  document.getElementById('people-groups').innerHTML=order.map(g=>{
    ...
    return `<div class="people-group" id="${slug(g)}"><div class="section-head people-group-head"><div><h2>${BERL.esc(g)}</h2></div></div><div class="grid-3">${list.map(m=>`<a class="card person-card" href="member.html?id=${encodeURIComponent(m.slug||m.id)}"><div class="portrait"><img src="${BERL.esc(m.photo||'assets/images/default-avatar.svg')}" alt="${BERL.esc(m.name)}" onerror="BERL.photo(this)"></div><div class="person-copy">${m.example?'<span class="example-badge">Placeholder</span>':''}<h3>${BERL.esc(m.name)}</h3><div class="role">${BERL.esc(m.position)}</div><p>${BERL.esc((m.research_interests||[]).slice(0,3).join(' · '))}</p></div></a>`).join('')}</div></div>`;
  }).join('');

  const alumni=members.filter(m=>m.group==='Alumni').sort((a,b)=>(a.sort_order||0)-(b.sort_order||0)||String(a.name).localeCompare(String(b.name)));
  ...
  const render=()=>{
    ...
    const filtered=alumni.filter(m=>[m.name,m.affiliation,m.email,m.position].some(v=>String(v||'').toLowerCase().includes(q)));
    ...
    if(count)count.textContent=alumni.length?`${filtered.length} of ${alumni.length}`:'0';
  };
