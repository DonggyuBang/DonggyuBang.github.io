document.addEventListener('DOMContentLoaded',async()=>{
  ...
  function imagePreview(url,label='Current image'){
    ...
    box.innerHTML=safe?`<div class="content-image-preview-label">${esc(label)}</div><img src="${esc(safe)}" alt="Content image preview" loading="lazy"><div class="content-image-preview-url">${esc(safe)}</div>`:'<div class="content-image-preview-empty">No image assigned.</div>';
  }
  async function load(){
    status('Loading…');
    let query=sb.from(table).select('*');
    if(type==='projects')query=query.order('status',{ascending:false});
    query=query.order('sort_order',{ascending:true}).order('created_at',{ascending:true});
    const {data,error}=await query;
    if(error)throw error;
    rows=data||[];
    const selectedId=current?.id;
    current=selectedId?rows.find(r=>Number(r.id)===Number(selectedId))||null:null;
    if(current)open(current.id);
    else if(rows.length)open(rows[0].id);
    else{render();$('content-manage-editor').hidden=true}
    status(`${rows.length} ${type==='research'?'research areas':'projects'}`);
  }
  function render(){const list=$('content-manage-list');list.innerHTML=rows.map(r=>`<button class="content-manage-row ${current?.id===r.id?'active':''}" type="button" data-id="${r.id}"><small>${r.is_published?'Published':'Draft'} · ${type==='projects'?esc(r.status||'Ongoing')+' · ':''}Order ${r.sort_order??0}${type==='research'&&r.code?' · '+esc(r.code):''}</small><strong>${esc(r.title)}</strong>${type==='research'&&r.tag?`<span>${esc(r.tag)}</span>`:''}</button>`).join('')||'<div style="padding:24px;color:#8fa7b1">No items.</div>';list.querySelectorAll('[data-id]').forEach(b=>b.onclick=()=>open(Number(b.dataset.id)))}
  ...
  async function upload(file){
    ...
    if(!current?.id)throw new Error(`Select a ${type==='research'?'research area':'project'} before uploading an image.`);
  ...
  async function save(){
    ...
    try{
      ...
      status(`Saving ${type==='research'?'research area':'project'}…`);
      ...
      if(String(data.image||'')!==String(image||''))throw new Error(`${type==='research'?'Research area':'Project'} saved, but the image URL was not persisted. Please retry.`);
  ...
  async function del(){if(!current||!confirm(`Delete “${current.title}”? This cannot be undone.`))return;const {error}=await sb.from(table).delete().eq('id',current.id);if(error)throw error;rows=rows.filter(r=>r.id!==current.id);current=null;if(rows.length)open(rows[0].id);else{$('content-manage-editor').hidden=true;render()}status('Deleted')}
