document.addEventListener('DOMContentLoaded',async()=>{
  try{
    const[cfg,metrics,pubs,research,initialNews]=await Promise.all([
    ...
    const sortNews=rows=>[...(rows||[])].sort((a,b)=>
    ...
    document.getElementById('hero-stats').innerHTML=[
      ...
      ['Environmental Engineering','Treatment & remediation'],
      ['Microbial','Biotechnology'],
      ['AI','Prediction & optimization']
    ].map(([a,b])=>`<span class="hero-stat">${a} · ${b}</span>`).join('');

    const heroImages=[
      'assets/images/generated/hero-clean-tech.png',
      'assets/images/generated/hero-landscape.png',
      'assets/images/generated/research-bioenergy.png',
      'assets/images/generated/research-remediation.png',
      'assets/images/generated/research-microbial.png',
      'assets/images/generated/research-ai.png'
    ];
    ...
    let hi=0;
    const hs=[...document.querySelectorAll('.hero-slide')];
    const hd=[...document.querySelectorAll('.hero-dot')];
    const setHero=i=>{
      hi=(i+hs.length)%hs.length;
      hs.forEach((e,j)=>e.classList.toggle('active',j===hi));
      hd.forEach((e,j)=>e.classList.toggle('active',j===hi));
      document.getElementById('hero-count').textContent=`${String(hi+1).padStart(2,'0')} / ${String(hs.length).padStart(2,'0')}`;
    };
    let ht;
    const resetHero=()=>{
      clearInterval(ht);
      ht=setInterval(()=>setHero(hi+1),6200);
    };
    ...
    let latest=sortNews(initialNews).slice(0,5);
    let newsSignature='';
    let ni=0;
    let nt;
    ...
    if('IntersectionObserver'in window){
      const io=new IntersectionObserver(es=>es.forEach(e=>{
        ...
        const step=n=>{
          const q=Math.min(1,(n-s)/1000);
          el.textContent=BERL.fmt(Math.round(t*(1-Math.pow(1-q,3))));
          if(q<1)requestAnimationFrame(step);
        };
        requestAnimationFrame(step);
        io.unobserve(el);
      }),{threshold:.35});
      document.querySelectorAll('[data-count]').forEach(x=>io.observe(x));
    }
